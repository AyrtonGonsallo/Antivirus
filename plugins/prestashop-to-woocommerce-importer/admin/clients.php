...
function presta_import_clients_page() {


    if (isset($_POST['upload_clients'])) {

        check_admin_referer('presta_upload_clients');

        if (
            empty($_FILES['clients_csv']['tmp_name']) ||
            $_FILES['clients_csv']['error'] !== UPLOAD_ERR_OK
        ) {
        ...
        $file = $directory . '/clients.csv';

        if (move_uploaded_file(
            $_FILES['clients_csv']['tmp_name'],
            $file
        )) {
    ...
    if (isset($_POST['import_clients'])) {
        ...
        check_admin_referer('presta_import_clients');
        ...
        presta_import_clients($line_start, $line_end);
    }
    ...
    <div class="wrap">
        ...
        <form method="post" enctype="multipart/form-data">
            ...
            <?php wp_nonce_field('presta_upload_clients'); ?>

            <h2>1. Upload des clients</h2>
        ...
        <form method="post">
            ...
            <?php wp_nonce_field('presta_import_clients'); ?>

            <h2>2. Import des clients</h2>
...
function presta_import_clients($line_start, $line_end) {

     error_log('line_start : '  . $line_start.' - line_end : '.$line_end);

        $upload_dir = wp_upload_dir();

        $file = $upload_dir['basedir'] . '/presta-import/clients.csv';

        if (!file_exists($file)) {

            echo '<div class="notice notice-error">
                    <p>Le fichier clients.csv est introuvable.</p>
                </div>';
        ...
        while (($row = fgetcsv($handle, 0, ';', '"', '')) !== false) {
            ...
            update_user_meta( $user_id, 'new_account_regime_tva', $new_account_regime_tva );
